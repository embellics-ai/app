...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[Server Startup] Failed to cleanup invitation passwords:",
      error
    );
  }
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (!user) {
        const pendingInvitation = await storage.getPendingInvitationByEmail(
          email
        );
          ...
          if (pendingInvitation.role === "client_admin") {
            ...
            if (!tenantId) {
              if (!pendingInvitation.companyName) {
                console.error(
                  "Missing company name in invitation:",
              ...
              console.log(
                `Created new tenant for first client_admin: ${tenantId}`
              );
            } else {
            ...
            const existingWidgetConfig = await storage.getWidgetConfig(
              tenantId
            );
            if (!existingWidgetConfig) {
              const companyName =
                pendingInvitation.companyName || "Your Company";
              ...
              console.log(
                `Created default widget config for tenant: ${tenantId}`
              );
            } else {
              console.log(
                `Widget config already exists for tenant: ${tenantId}`
              );
            }
          }
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!user) {
          return res
            .status(401)
            .json({ error: "User not found or invalid session" });
        }
        ...
        if (!isPlatformAdmin && user.tenantId !== tokenTenantId) {
          return res
            .status(401)
            .json({ error: "Tenant mismatch - invalid session" });
        }
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    try {
      ...
      await storage.updateClientUserPassword(
        matchedToken.userId,
        hashedPassword
      );
      ...
      console.log(
        `[Reset Password] Password successfully reset for user: ${matchedToken.userId}`
      );
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (existingUser) {
          console.log(
            `[Invite User] ❌ User already exists: ${data.email}`,
            existingUser
          );
          return res
            .status(400)
        ...
        const existingInvitation = await storage.getPendingInvitationByEmail(
          data.email
        );
        if (existingInvitation) {
          console.log(
            `Deleting old pending invitation for ${data.email} before creating new one`
          );
        ...
        if (data.role === "client_admin") {
          ...
          if (data.tenantId) {
            ...
            console.log(
              `[Invite User] Inviting to existing tenant: ${tenantId}`
            );
          } else {
  ...
  app.delete(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        console.log(
          `[Delete Invitation] ✓ Successfully deleted invitation: ${invitationId}`
        );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!widgetConfig) {
          ...
          console.log(
            `[Update Retell API Key] Creating new widget config for tenant: ${tenantId}`
          );
        ...
        if (!widgetConfig) {
          return res
            .status(500)
            .json({ error: "Failed to update Retell API key" });
        }
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (userToDelete.email === "admin@embellics.com") {
          ...
          if (!securityAnswer) {
            return res.status(403).json({
              error:
                "Platform owner account is protected. Security answer required.",
          ...
          if (!validSecurityAnswer) {
            return res.status(403).json({
              error:
                "Incorrect security answer. Platform owner account cannot be deleted.",
              securityQuestion: "What is the platform owner's secret code?",
            });
          }
        ...
        if (userToDelete.isPlatformAdmin) {
          return res
            .status(403)
            .json({ error: "Cannot delete platform admin users" });
        }

        // Only allow deleting client_admin users
        if (
          userToDelete.role !== "client_admin" &&
        ...
        console.log(
          `[Delete User] Deleting user: ${userToDelete.email} (ID: ${userId})`
        );

        // Delete the user's tenant if they have one (cascade delete for client admins)
        if (userToDelete.tenantId) {
          console.log(
            `[Delete User] Deleting associated tenant: ${userToDelete.tenantId}`
          );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (req.user!.email !== "admin@embellics.com") {
          return res
            .status(403)
            .json({ error: "Only platform owner can reset passwords" });
        }
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        const cleanedCount = await storage.cleanupExpiredInvitationPasswords();
        console.log(
          `[Password Cleanup] Cleaned up ${cleanedCount} expired plaintext passwords`
        );
        res.json({
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (userToDelete.tenantId !== tenantId) {
          return res
            .status(403)
            .json({ error: "Cannot delete users from other tenants" });
        }
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        // Get daily analytics
        const dailyData = await storage.getDailyAnalytics(
          tenantId,
          startDate,
          endDate
        );
        ...
        // Calculate totals
        const totalConversations = dailyData.reduce(
        ...
        const totalUsers = dailyData.reduce(
          (sum, day) => sum + day.uniqueUsers,
          0
        );
        const avgInteractions =
          dailyData.length > 0
            ? Math.round(
                dailyData.reduce((sum, day) => sum + day.avgInteractions, 0) /
                  dailyData.length
              )
            : 0;
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if ("retellApiKey" in req.body || "retellAgentId" in req.body) {
          return res.status(400).json({
            error:
              "Client admins cannot configure Retell AI credentials. Contact platform admin.",
          });
        }
        ...
        res
          .status(500)
          .json({ error: "Failed to create widget configuration" });
      }
    }
  );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if ("retellApiKey" in req.body || "retellAgentId" in req.body) {
          return res.status(400).json({
            error:
              "Client admins cannot modify Retell AI credentials. Contact platform admin.",
          });
        }
        ...
        // Update config
        const config = await storage.updateWidgetConfig(
          tenantId,
          validatedData
        );
        ...
        res
          .status(500)
          .json({ error: "Failed to update widget configuration" });
      }
    }
  );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!widgetConfig?.retellApiKey) {
          console.log(
            `[Analytics] No Retell API key configured for tenant: ${tenantId}`
          );
          return res.json({
            totalCalls: 0,
            completedCalls: 0,
        ...
        console.log(
          `[Analytics] Fetching account-wide calls for tenant: ${tenantId}`
        );
        console.log(
        ...
        try {
          ...
          try {
            ...
            activeAgentIds = new Set(
              activeAgents.map((agent: any) => agent.agent_id)
            );
            ...
            console.log(
              `[Analytics] Found ${activeAgentIds.size} active agents in account`
            );
          } catch (agentError) {
            console.warn(
          ...
          do {
            ...
            console.log(
              ...
              JSON.stringify(
                { limit: maxLimit, has_pagination_key: !!paginationKey },
                null,
                2
              )
            );

            const response: any = await tenantRetellClient.call.list(
              pageParams
            );
            ...
            if (Array.isArray(response) && response.length > 0) {
              ...
              if (response.length === maxLimit) {
                ...
                console.log(
                  `[Analytics] Last page reached (got ${response.length} < ${maxLimit})`
                );
              }
            } else {
          ...
          if (activeAgentIds.size > 0) {
            ...
            calls = calls.filter((call: any) =>
              activeAgentIds.has(call.agent_id)
            );
            const callsRemoved = callsBeforeFilter - calls.length;

            if (callsRemoved > 0) {
              console.log(
                `[Analytics] Filtered out ${callsRemoved} calls from deleted agents`
              );
              console.log(
          ...
          if (Object.keys(filter).length > 0 && filter.start_timestamp) {
            ...
            calls = calls.filter((call: any) => {
              ...
              if (
                filter.start_timestamp.gte &&
                call.start_timestamp < filter.start_timestamp.gte
              ) {
                return false;
              }
              if (
                filter.start_timestamp.lte &&
                call.start_timestamp > filter.start_timestamp.lte
              ) {
                return false;
              }
              return true;
          ...
          if (calls.length > 0) {
            const agentIds = Array.from(
              new Set(calls.map((c: any) => c.agent_id))
            );
            console.log(
            ...
            console.log(
              `[Analytics] Call direction breakdown:`,
              directionCounts
            );
            ...
            if (calls.length > 0) {
              ...
              if (timestamps.length > 0) {
                ...
                console.log(
                  `[Analytics] Actual call date range: ${earliest} to ${latest}`
                );
              }
            }
          }
          ...
          console.error(
            "[Retell] Error details:",
            JSON.stringify(retellError, null, 2)
          );
        ...
        for (const call of calls) {
          ...
          if (call.direction) {
            directionBreakdown[call.direction] =
              (directionBreakdown[call.direction] || 0) + 1;
          }
        ...
        const successRate =
          completedCalls > 0 ? (successfulCalls / completedCalls) * 100 : 0;
        const pickupRate =
          completedCalls > 0 ? (pickedUpCalls / completedCalls) * 100 : 0;
        const transferRate =
          completedCalls > 0 ? (transferredCalls / completedCalls) * 100 : 0;
        const voicemailRate =
          completedCalls > 0 ? (voicemailCalls / completedCalls) * 100 : 0;

        // Format sentiment breakdown as percentages
        const sentimentBreakdown: Record<string, number> = {};
        for (const [sentiment, count] of Object.entries(sentimentCounts)) {
          sentimentBreakdown[sentiment] =
            completedCalls > 0 ? (count / completedCalls) * 100 : 0;
        }
        ...
        const dailyMetricsArray = Object.entries(dailyMetrics)
          .map(([date, metrics]: [string, any]) => ({
            ...
            transferRate:
              metrics.totalCalls > 0
                ? Math.round(
                    (metrics.transferredCalls / metrics.totalCalls) * 100
                  )
                : 0,
            voicemailRate:
              metrics.totalCalls > 0
                ? Math.round(
                    (metrics.voicemailCalls / metrics.totalCalls) * 100
                  )
                : 0,
            avgDuration:
              metrics.totalCalls > 0
        ...
        const agentMetricsArray = Object.entries(agentMetrics).map(
          ([agentId, metrics]: [string, any]) => ({
            ...
            pickupRate:
              metrics.totalCalls > 0
                ? (metrics.pickedUpCalls / metrics.totalCalls) * 100
                : 0,
            transferRate:
              metrics.totalCalls > 0
                ? (metrics.transferredCalls / metrics.totalCalls) * 100
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!widgetConfig?.retellApiKey) {
          console.log(
            `[Platform Analytics] No Retell API key configured for tenant: ${tenantId}`
          );
          return res.json({
            totalCalls: 0,
            completedCalls: 0,
        ...
        console.log(
          `[Platform Analytics] Fetching account-wide calls for tenant: ${tenantId}`
        );
        console.log(
          `[Platform Analytics] Filter criteria:`,
          JSON.stringify(filter, null, 2)
        );
        ...
        try {
          ...
          do {
            ...
            if (pageCount >= 100) {
              console.warn(
                `[Platform Analytics] Reached pagination safety limit`
              );
              break;
            }
          } while (paginationKey);
          ...
          console.error(
            "[Platform Analytics] Error fetching calls:",
            retellError
          );
          return res.json({
            totalCalls: 0,
        ...
        for (const call of calls) {
          ...
          const sentiment = call.call_analysis?.user_sentiment || "Unknown";
          sentimentBreakdown[sentiment] =
            (sentimentBreakdown[sentiment] || 0) + 1;

          const reason = call.disconnection_reason || "Unknown";
          disconnectionReasons[reason] =
            (disconnectionReasons[reason] || 0) + 1;

          const status = call.call_status || "Unknown";
          callStatusBreakdown[status] = (callStatusBreakdown[status] || 0) + 1;

          const direction = call.direction || "Unknown";
          directionBreakdown[direction] =
            (directionBreakdown[direction] || 0) + 1;
        ...
        const averageDuration =
          completedCalls > 0 ? totalDuration / completedCalls : 0;
        const averageLatency =
          callsWithLatency > 0 ? totalLatency / callsWithLatency : 0;
        const successRate =
          completedCalls > 0 ? (successfulCalls / completedCalls) * 100 : 0;
        const pickupRate =
          completedCalls > 0 ? (pickedUpCalls / completedCalls) * 100 : 0;
    ...
    ws.on("message", (msg: string) => {
      try {
        ...
        if (!authenticated) {
          ...
          console.log(
            `[WebSocket] Client authenticated for tenant: ${tenantId}`
          );

          // Send authentication success message
          ws.send(
            JSON.stringify({ type: "auth:success", payload: { tenantId } })
          );
          return;
        }
...
async function getRetellAgentResponse(
  ...
  try {
    ...
    if (!widgetConfig?.retellAgentId) {
      console.error(
        `[Retell] Tenant ${tenantId} has no configured Retell agent ID`
      );
      throw new Error(
    ...
    if (!retellChatId) {
      console.log(
        "[Retell] Creating new chat session for conversation:",
        conversationId
      );
    ...
    if (!lastAssistantMessage) {
      ...
      const hasToolMessages = messages.some(
        (msg: any) =>
          msg.role === "tool_call_invocation" || msg.role === "tool_call_result"
      );
      ...
      console.error(
        "[Retell] No agent/assistant message found and no tool processing detected"
      );
      console.error(
        "[Retell] Full response:",
...
- Latest user message: ${
      userMessages[userMessages.length - 1]?.content || "N/A"
    }
